...
class User extends Authenticatable
{
    ...
    public function accepted(User $user)
    {
        if ($this->status == "public") {
            ...
            return (bool) DB::table('follows')->where('user_id', $user->id)->where('following_user_id', $this->id)->where('accepted', true)->count();
        }
    }
    public function followReq()
    {
        if ($this->status == "private") {
            return $this->followers()->where('following_user_id', $this->id)->where('accepted', false)->latest()->paginate(5);
        }
        return null;
    }
    public function pendingFollowReq()
    {
        return $this->follows()->where('user_id', $this->id)->where('accepted', false)->latest()->paginate(5);
    }
    ...
    {
        return DB::table('follows')->where('user_id', $user->id)->where('following_user_id', $this->id)->update(['accepted' => $state,]);
    }
